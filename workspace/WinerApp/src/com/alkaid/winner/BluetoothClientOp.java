    ...
    private class SearchDevicesReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {  
                ...
                String isPair= device.getBondState() == BluetoothDevice.BOND_BONDED?"已配�?":"未配�?";
...
//                }  
    ...
    private void searchDevice() {
        ...
        if(Constants.D) Log.d(TAG,"�?始搜�?,搜索重试次数="+search_count);
        Util.toast(ctx, "�?始搜�?,搜索重试次数="+search_count);
    }
...
//    	getUUID(device);
    ...
    private int match(BluetoothDevice device){
        if (device.getBondState() == BluetoothDevice.BOND_NONE) { 
            ...
            if(matchSuccess){
    			if(Constants.D) Log.d(TAG,"调用配对方法成功，等待配对监�?... ");
    			Util.toast(ctx, "调用配对方法成功，等待配对监�?...");
    ...
    private boolean matchRetry(BluetoothDevice device){
		if(Constants.D) Log.d(TAG,"�?始配�?...，配对重试次�?:"+match_count);
		Util.toast(ctx, "�?始配�?...，配对重试次�?:"+match_count);
